...
impl<'a> HistoryList<'a> {
    ...
    fn get_items_bounds(&self, selected: usize, offset: usize, height: usize) -> (usize, usize) {
        ...
        let max_scroll_space =
            height.saturating_sub(scroll_margin).min(self.history.len() - selected);
...
impl DrawState<'_> {
    ...
    fn render_row(&mut self, h: &History) {
        ...
        let expand_width = self.list_area.width.saturating_sub(padding_width + fixed_width);
